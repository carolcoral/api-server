    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(authz -> authz
                        ...
                        // AI代理接口（OpenAI兼容）- 通过API Key鉴权，URL层面放行（必须放在 /api/ai/** 之前）
                        .requestMatchers("/api/ai/v1/**").permitAll()
                        // AI功能接口 - 需要认证（所有登录用户均可使用）
                        .requestMatchers("/api/ai/**").authenticated()
                        // AI用户自助服务接口 - 需要认证（所有登录用户均可使用）
                        .requestMatchers("/api/user/ai/**").authenticated()
                        // 用户信息接口 - 需要认证
                        .requestMatchers(
                                "/api/users/profile",
                ...
                .exceptionHandling(exceptions -> exceptions
                        .authenticationEntryPoint((request, response, authException) -> {
                            log.warn("未认证访问被拒绝: {} {}", request.getMethod(), request.getRequestURI());
