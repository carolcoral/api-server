...
import com.carolcoral.apiserver.service.PermissionScanner;
...
public class PermissionController {
    ...
    public ApiResponse<List<Map<String, Object>>> scanPermissions() {
        // 按前端侧边栏菜单顺序排序分组（与 DashboardLayout.vue 一致）
        List<String> groupOrder = java.util.List.of(
            "仪表盘管理", "项目管理", "项目成员管理", "接口管理", "自定义代码模板管理", "请求录制与回放",
            "AI聊天",
            "统计管理", "请求日志",
            "用户管理", "角色管理", "权限管理",
            "AI用户自助",
            "邮件模板管理", "AI配置", "AI配置管理", "AI管理", "AI代理", "系统配置", "系统信息", "系统管理",
            "认证管理"
        );
        ...
        List<Map<String, Object>> result = permissionScanner.getAllScannedPermissions().stream()
            ...
            .sorted((a, b) -> {
                String ga = (String) a.getOrDefault("groupName", "");
                String gb = (String) b.getOrDefault("groupName", "");
                int idxA = groupOrder.indexOf(ga);
                int idxB = groupOrder.indexOf(gb);
    ...
    public ApiResponse<Map<String, Object>> getPermissionSuggestions() {
        ...
        List<Map.Entry<String, List<Map<String, Object>>>> sortedEntries = new ArrayList<>(groupedByGroup.entrySet());
        sortedEntries.sort((a, b) -> {
            int idxA = groupOrder.indexOf(a.getKey());
            int idxB = groupOrder.indexOf(b.getKey());
            if (idxA >= 0 && idxB >= 0) return Integer.compare(idxA, idxB);
            if (idxA >= 0) return -1;
            if (idxB >= 0) return 1;
