...
public class MockController {
    ...
    public ResponseEntity<Object> handleMockRequest(
        ...
        try {
            ...
            // fullPath 为 /api/api-server/test/login
            // 构建路径前缀 /api/api-server/{projectCode}/
            String pathPrefix = "/api/api-server/" + projectCode + "/";
            String path;

            if (fullPath.startsWith(pathPrefix)) {
                path = fullPath.substring(pathPrefix.length() - 1); // 保留开头的 /
            } else if (fullPath.startsWith("/api/api-server/")) {
                String mockPathPrefix = "/api/api-server/" + projectCode;
            ...
            } else if (servletPath.startsWith("/api-server/")) {
                ...
                path = fullPath.startsWith("/api/api-server/")
                    ? fullPath.substring("/api/api-server/".length())
                    : fullPath;
            }
    ...
    private MockRequest buildMockRequest(String projectCode, String path, String method, HttpServletRequest request) {
        ...
        // 从请求URI中提取api-server后的部分
        String afterMockServer = "";
        if (pathWithoutContext.startsWith("/api-server/")) {
            int idx = pathWithoutContext.indexOf("/", "/api-server/".length());
            if (idx != -1) {
