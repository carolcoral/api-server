...
import com.carolcoral.apiserver.entity.CustomCodeTemplate;
import com.carolcoral.apiserver.entity.EmailTemplate;
import com.carolcoral.apiserver.repository.CustomCodeTemplateRepository;
...
public class AiService {
    ...
    private String buildEmailTemplatePrompt(String templateType, String templateName,
        ...
        prompt.append("\n");
        prompt.append("要求：\n");
        prompt.append("1. 邮件主题简洁明了，包含系统名称【API Server】前缀\n");
    ...
    private String fixImports(String sourceCode, String transformerType) {
        ...
        requiredImports.add("import com.carolcoral.apiserver.dto.MockRequest;");
        requiredImports.add("import com.carolcoral.apiserver.dto.MockResponseDTO;");
        requiredImports.add("import com.carolcoral.apiserver.plugin.CustomResponseTransformer;");
    ...
    private java.io.BufferedReader doStreamRequest(AiConfig config, List<Map<String, String>> messages, String modelOverride) throws Exception {
        ...
        // 如果 API 地址指向系统自身，内部路由到 AiProxyService，避免 HTTP 循环
        if (isSystemSelfUrl(config.getApiUrl())) {
            log.info("AI Stream 内部路由: model={}, messagesCount={}", model, messages.size());
            return aiProxyService.processStreamChatInternal(model, messages);
        }
    ...
    private String buildSystemPrompt(String question) {
        ...
        if (readme != null && !readme.isBlank()) {
            ...
            if (!snippet.isBlank()) {
                context.append("=== API Server 系统说明 ===\n").append(snippet).append("\n\n");
                hasContext = true;
            }
        }
        ...
        StringBuilder prompt = new StringBuilder();
        prompt.append("你是 API Server 的智能助手。\n\n");
        ...
        if (hasContext) {
            ...
            prompt.append("我将为你回答关于 API Server 或相关技术的问题。请基于你的知识库提供准确、简洁的回答，优先使用中文。\n");
        }
        ...
        prompt.append("4. 如果用户提问与 API Server 完全无关，可以简短回应但不要过度展开\n");
    ...
    private String extractCoreInfo(String readme) {
        ...
        for (String line : lines) {
            ...
            if (trimmed.startsWith("<h1") || trimmed.startsWith("API Server") && !trimmed.startsWith("API Server 有哪些")) {
    ...
    private String resolveAppVersion() {
        try {
            ...
            InputStream is = getClass().getClassLoader().getResourceAsStream("META-INF/maven/com.carolcoral/api-server/pom.properties");
            if (is != null) {
    ...
    private List<String> generateSuggestionsWithAI(AiConfig config, String readme, String changelog) {
        try {
            String prompt = "你是 API Server 的智能助手。请严格根据以下系统文档（README 和 CHANGELOG），" +
                    "生成 6 个用户最可能提问的建议问题。\n\n" +
                    "重要约束：\n" +
